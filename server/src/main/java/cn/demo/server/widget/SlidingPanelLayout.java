...
import cn.demo.server.BuildConfig;
...
//        LogTool.log("SlidingPanelLayout setTx " + contentView.getTranslationX());
        if (enableAlpha)
            contentView.setAlpha(Math.max(0.1f, decelerateInterpolator.getInterpolation(mPanelPositionRatio)));
//        LogTool.log("setAlpha . ratio="+ mPanelPositionRatio +", alpha="+contentView.getAlpha());
        if (drag != null) {
            drag.overlayScrollChanged(mPanelPositionRatio);
        }
    }
        ...
        switch (motionEvent.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mTouchState == 1) {
                    ...
                    LogTool.log("onTouchEvent.move. setPanelX = " + (moveX + x));
